...
phoneIp.addEventListener("input", ()=>{
    ...
    if(phonelength > 10){
        phoneIp.value = phoneIp.value.replace(/(\d{2})(\d{5})(\d{1,4})/, "($1) $2-$3");
    } else if (phonelength > 2) {
        phoneIp.value = phoneIp.value.replace(/(\d{2})(\d{1,5})/, "($1) $2");
    }
})
...
function message(){
    ...
    if (cpfIp.value == "" || cpfIp.value.length < 14){
        cpfIp.style.borderColor = "Red"
        cpfIp.classList.add("error")
    }else if(phoneIp.value == "" || phoneIp.value.length < 15){
